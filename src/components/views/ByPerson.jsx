/**
 * ByPerson.jsx
 * View page: "Items by Person" with Chart / Table toggle.
 * Chart view renders ByPersonChart; Table view renders expandable accordion.
 */
...
import ByPersonChart from '../charts/ByPersonChart.jsx';
...
export default function ByPerson() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const item of nonArchivedItems) {
      ...
      if (item.dueDate && OPEN_STATUSES.has(item.status)) {
        ...
        if (daysUntil !== null) {
          if (map[person].nextDue === null || item.dueDate < map[person].nextDue) {
            map[person].nextDue = item.dueDate;
          }
        }
      }
    }
  ...
  // Display name: translate "Unassigned" if needed
  const displayName = (name) => (name === 'Unassigned' ? t('view_unassigned') : name);

  return (
    <div className="bg-[#F8F9FA] min-h-screen p-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold tracking-tight text-boronia-navy">{t('view_by_person')}</h1>
      ...
      {activeTab === 'chart' ? (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <ByPersonChart items={nonArchivedItems} />
        </div>
      ) : (
