/**
 * ByDepartmentChart.jsx
 * Recharts PieChart (donut variant) showing open item count per department.
 * Below the chart: ranked list table with Rank, Department, Open, Overdue, % of Total.
 */
...
export default function ByDepartmentChart({ items = [] }) {
  ...
  const data = useMemo(() => {
    ...
    for (const item of items) {
      if (!item.isArchived && OPEN_STATUSES.has(item.status)) {
        ...
        if (!grouped[dept]) grouped[dept] = { name: dept, open: 0, overdue: 0 };
        grouped[dept].open += 1;
        if (isOverdue(item.dueDate, item.status)) {
          grouped[dept].overdue += 1;
        }
      }
    }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="text-left py-2 px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('field_department')}</th>
