/**
 * ByPersonChart.jsx
 * Recharts PieChart (donut variant) showing open item count per assignee.
 * Below the chart: ranked list table with Rank, Name, Open, Overdue, % of Total.
 */
...
const CHART_COLORS = [
  '#3498DB', '#E67E22', '#E74C3C', '#2ECC71', '#9B59B6',
...
export default function ByPersonChart({ items = [] }) {
  ...
  const data = useMemo(() => {
    ...
    for (const item of items) {
      if (!item.isArchived && OPEN_STATUSES.has(item.status)) {
        ...
        if (!grouped[person]) grouped[person] = { name: person, open: 0, overdue: 0 };
        grouped[person].open += 1;
        if (isOverdue(item.dueDate, item.status)) {
          grouped[person].overdue += 1;
        }
      }
    }
  ...
  const chartData = useMemo(
    () => data.map((d) => ({ name: d.name === 'Unassigned' ? t('view_unassigned') : d.name, value: d.open })),
    [data, t]
  );
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="text-left py-2 px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('field_assigned_to')}</th>
            ...
            {data.map((d, idx) => (
              <tr key={d.name} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-2 px-2 font-medium text-boronia-navy">
                  {d.name === 'Unassigned' ? t('view_unassigned') : d.name}
                </td>
